 ...
 * Once {@link HttpProxy} is finished and is using Grizzly Client, than {@link SimpleHttpProxy} should be deprecated.
 *
...
public class SimpleHttpProxy implements Adapter {
    ...
    public void service(Request request, Response response) throws Exception {
        ...
        httpClient.getHostConfiguration().setHost(targetHost, targetPort, request.scheme().toString());
        ...
        transfer(httpMethod.getResponseBodyAsStream(), response.getOutputBuffer(), response);
    }

    private void transfer(InputStream responseBodyAsStream, OutputBuffer outputBuffer, Response response) throws IOException {
        ...
        do {
            ...
            if (read != -1) {
                ...
                if (logger.isLoggable(Level.FINEST)) {
                    logger.log(Level.FINEST, "Received Start\n" + new String(bc.getBuffer()));
                }
            }
        } while (read != -1);
        response.finish();
    ...
    private void populateHeaders(Header[] responseHeaders, Response response) {
        if (logger.isLoggable(Level.FINEST)) {
            logger.entering(this.getClass().getName(), "populateHeaders", new Object[]{responseHeaders, response});
        }
        for (Header responseHeader : responseHeaders) {
            if (logger.isLoggable(Level.FINEST)) {
                logger.log(Level.FINEST, MessageFormat.format("Populating header name: {0}, value: {1}", responseHeader.getName(), responseHeader.getValue()));
            }
            response.setHeader(responseHeader.getName(), responseHeader.getValue());
        }
    }

    private HttpMethod createMethod(Request request) throws URIException {
        HttpMethod method = null;

        String uri = request.unparsedURI().toString();
        String methodName = request.method().toString();
        if (logger.isLoggable(Level.FINER)) {
            logger.log(Level.FINER, MessageFormat.format("Creating reqeuest for method: {0}, URI: {1}", methodName, uri));
        }
    ...
    private NameValuePair[] converParameters(Parameters parameters) {
        ...
        while (parameterNames.hasMoreElements()) {
            ...
            result.add(new NameValuePair(name, parameters.getUndecodedParameter(name)));
        }
